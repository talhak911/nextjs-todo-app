...
export const mailer= async ({ email, emailType, userId }: any) => {
    try {
          ...
          const getMailHtml = () => {
            ...
            return (
                ...
                return(
                    ...
                    or copy and paste the link below in your browser. <br> ${process.env.DOMAIN}/reset-password?token=${token}&email=${email}
                    </p>`
                )
            }
                ...
                return(
                    `<p>Click <a href="${process.env.DOMAIN}/change-email?token=${token}&email=${email}">here</a> to change your email address
                    or copy and paste the link below in your browser. <br> ${process.env.DOMAIN}/change-email?token=${token}
                    </p>`
                )
            }
        };
        ...
        const mailData = {
            ...
            to: [email],
            subject: emailType === "VERIFY"||emailType ===  "CHANGE" ? "Verify your email" : "Reset your password",
            html: getMailHtml()
